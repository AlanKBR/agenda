        ...
        (function initMiniCalendar() {
            try {
                ...
                const mini = new FullCalendar.Calendar(miniEl, {
                    ...
                    // Same event source and params, but focus on date range; backend should return all-day events as allDay=true
                    eventSources: [{
                        url: '/events',
                        method: 'GET',
                        extraParams: function () {
                    ...
                    dateClick: (info) => {
                        try { calendar.gotoDate(info.date); } catch (e) { }
                    },
                    dayCellDidMount: (arg) => {
                        try {
                ...
                function rebuildMiniIndicators() {
                    try {
                        const viewApi = mini.view;
                        const start = new Date(viewApi.currentStart);
                        const end = new Date(viewApi.currentEnd);
                        ...
                        miniEl.querySelectorAll('.fc-daygrid-day').forEach(cell => {
                            ...
                            if (!hasAny && !hasHolidayBg && !isToday) {
                                ...
                                ov.style.inset = '0';
                                // usar amarelo suave padrão independentemente do bg de hoje
                                ov.style.background = '#fff3cd';
                                // reduzir intensidade em dias de meses não atuais
                                ov.style.opacity = isOtherMonth ? '0.35' : '0.6';
                ...
                // manter mini em sincronia com a data do calendário principal
                calendar.on('datesSet', () => {
                    try {
                        mini.gotoDate(calendar.getDate());
                        // refresh events/holiday state
                        mini.refetchEvents();
                        // re-render para atualizar destaque da seleção
                        mini.render();
                ...
                mini.on('eventsSet', () => { try { rebuildMiniIndicators(); } catch (e) { } });
            } catch (e) { /* noop */ }
        })();
